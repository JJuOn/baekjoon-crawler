...
def get_cookies(userId,password):
    ...
    driver.find_element('name','login_user_id').send_keys(userId)
    driver.find_element('name','login_password').send_keys(password)
    driver.find_elements('id','submit_button')[0].click()
    print("="*25)
...
def get_source(problemList,option,userId,cookies,progress_bar):
    ...
    for problem in problemList:
        ...
        file=open('./{}/{}~{}/{}.{}'.format(dirname,int(problem)//1000*1000,(int(problem)//1000+1)*1000-1,problem,extension),'w', encoding='UTF-8')
        file.write(source)
        file.close()
